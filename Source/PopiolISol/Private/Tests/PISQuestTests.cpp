...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FPISQuestExclusionTest,"PopiolISol.Quests.ExclusionAndIdempotency",EAutomationTestFlags::EditorContext|EAutomationTestFlags::EngineFilter)
bool FPISQuestExclusionTest::RunTest(const FString&){
 auto* Q=NewObject<UPISQuestComponent>();
 TestTrue(TEXT("Start old trial"),Q->StartQuest(TEXT("old_trial_1")));
 TestTrue(TEXT("Finish old trial"),Q->FinishQuest(TEXT("old_trial_1")));
 TestFalse(TEXT("Cannot double finish"),Q->FinishQuest(TEXT("old_trial_1")));
 TestTrue(TEXT("Choose old camp"),Q->ChooseFaction(TEXT("stary_porzadek")));
 TestTrue(TEXT("New trial locked"),Q->IsQuestLocked(TEXT("new_trial_1")));
 TestFalse(TEXT("Cannot start locked quest"),Q->StartQuest(TEXT("new_trial_1")));
 return true;
}
#endif
